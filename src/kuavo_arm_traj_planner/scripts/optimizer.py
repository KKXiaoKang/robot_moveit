...
class Optimizer(Base):
    ...
    def retime_l_traj(self, traj: moveit_msgs.msg.RobotTrajectory) -> moveit_msgs.msg.RobotTrajectory:
        """重新计时轨迹优化
        
        :param traj: 待优化轨迹
        :return: 优化轨迹
        """
        traj = self.l_move_group.retime_trajectory(
    ...
    def retime_r_traj(self, traj: moveit_msgs.msg.RobotTrajectory) -> moveit_msgs.msg.RobotTrajectory:
        """重新计时轨迹优化
        
        :param traj: 待优化轨迹
        :return: 优化轨迹
        """
        traj = self.r_move_group.retime_trajectory(
            self.robot.get_current_state(),
            traj,
            velocity_scaling_factor=0.5,
            acceleration_scaling_factor=0.5,
