...
def save_traj(traj, filename):
    traj_dict = dict()
    traj_dict['Header_seq'] = traj.header.seq
    traj_dict['Header_stamp'] = traj.header.stamp.to_sec()
    traj_dict['Header_frame_id'] = traj.header.frame_id
    traj_dict['joint_names'] = traj.joint_names
    num_points = len(traj.points)
    traj_dict['num_points'] = num_points
    traj_dict['points_positions'] = []
    traj_dict['points_velocities'] = []
    traj_dict['points_accelerations'] = []
    traj_dict['points_effort'] = []
    traj_dict['points_time_from_start'] = []
    for i in range(num_points):
        traj_dict['points_positions'].append(traj.points[i].positions)
        traj_dict['points_velocities'].append(traj.points[i].velocities)
        traj_dict['points_accelerations'].append(traj.points[i].accelerations)
        traj_dict['points_effort'].append(traj.points[i].effort)
        traj_dict['points_time_from_start'].append(
            traj.points[i].time_from_start.to_sec())

    with open(filename, 'w') as f:
        yaml.dump(traj_dict, f)
