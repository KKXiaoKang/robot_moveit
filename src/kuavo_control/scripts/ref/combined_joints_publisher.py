...
def joint_states_relay(message):
...
if __name__ == "__main__":
    ...
    #                  joint_states_relay, queue_size=1)
    all_joint_names = rospy.wait_for_message("/joint_states", JointState)
    for i, name in enumerate(all_joint_names.name):
        joints_dict[name] = all_joint_names.position[i]
