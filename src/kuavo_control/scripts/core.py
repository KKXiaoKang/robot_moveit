...
class Core(Base):
    ...
    def goto_joint_config(self, joints: list) -> moveit_msgs.msg.RobotTrajectory:
        """前往关节目标位置
        
        最近一次轨迹保存在config/traj.json中
        ...
        traj = self.optimizer.retime_traj(traj)
        dump_traj("config/traj.json", traj)
        self.move_group.execute(traj)

        return traj
    
    def goto_pose_config(self, pose: geometry_msgs.msg.Pose) -> moveit_msgs.msg.RobotTrajectory:
        """前往笛卡尔目标位置
    ...
    def set_pick_config(self, pose: geometry_msgs.msg.Pose) -> moveit_msgs.msg.Grasp:
        ...
        grasp.grasp_pose.header.frame_id = self.planning_frame
    ...
    def set_place_config(self, pose: geometry_msgs.msg.Pose) -> moveit_msgs.msg.PlaceLocation:
        ...
        place.place_pose.header.frame_id = self.planning_frame
    ...
    def goto_pick_config(self, target: str, grasp: moveit_msgs.msg.Grasp) -> moveit_msgs.msg.RobotTrajectory:
        ...
        for i in range(self.config["num_planning"]):
            ...
            result = self.move_group.pick(target, grasp)
    ...
    def goto_place_config(self, target: str, place: moveit_msgs.msg.PlaceLocation) -> moveit_msgs.msg.RobotTrajectory:
        ...
        for i in range(self.config["num_planning"]):
            ...
            result = self.move_group.place(target, place)
            if result == moveit_msgs.msg.MoveItErrorCodes.SUCCESS:
                try:
                    traj_msg = rospy.wait_for_message(
                        "/move_group/display_planned_path",
    ...
    def open_gripper_config(self, posture: trajectory_msgs.msg.JointTrajectory) -> None:
        ...
        """
        posture.joint_names = self.config["gripper"]
    ...
    def goto_file_traj_config(self, path: str) -> moveit_msgs.msg.RobotTrajectory:
        """执行文件固定轨迹
        """
        traj = load_traj(path)
        self.move_group.execute(traj)
    ...
    def goto_file_joint_config(self, path: str) -> moveit_msgs.msg.RobotTrajectory:
        """执行文件固定轨迹点
        """
        joints = load_joints(path)
        traj = moveit_msgs.msg.RobotTrajectory()
        traj.joint_trajectory.header.frame_id = self.planning_frame
